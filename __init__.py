...
def api_get_manga_aggregate(manga_id):
    ...
    chapters = []
    volumes = api_request(
        f"{BASE_URL}/manga/{manga_id}/aggregate", params={"translatedLanguage[]": "en"}
    ).get("volumes", {})
...
def api_get_chapter_images(chapters):
    ...
    for chapter in tqdm(chapters, desc="Fetching chapter images", colour="blue"):
        chapter["images"] = api_request(f"{BASE_URL}/at-home/server/{chapter['id']}").get(
            "chapter", {}
        )
...
def process():
    ...
    try:
        selection = int(
            input("Enter the number of the manga you want to download (or 0 to skip): ")
        )
        if 1 <= selection <= len(manga_list):
            selected_manga_id = manga_list[selection - 1]["id"]
            chapters_with_images = api_get_chapter_images(
                api_get_manga_aggregate(selected_manga_id)
            )
            return {
                "title": manga_list[selection - 1]["attributes"]["title"]["en"],
                "detail": chapters_with_images,
            }
        else:
...
def generate_json(data):
    ...
    for detail in tqdm(data["detail"], desc="Generating JSON data", colour="green"):
        image_urls = [
            f"{IMAGE_URL}/{detail['images']['hash']}/{img}" for img in detail["images"]["data"]
        ]
        json_data.append(
            {
                "directory_path": f"{data['title']}/chapter {detail['chapter']}",
...
def download_images(data):
    for entry in data:
        manga_title, chapter = (
            entry["directory_path"].split("/")[0],
            entry["directory_path"].split("/")[-1].split(" ")[1],
        )
        ...
        file_paths = [
            ...
            / manga_title
            / f"chapter {chapter}"
            / f"{index + 1}.{url.split('.')[-1]}"
            for index, url in enumerate(entry["image_path"])
        ]

        # Check for missing images
        missing_images = [
            url for url, file_path in zip(entry["image_path"], file_paths) if not file_path.exists()
        ]
        ...
        if missing_images:
            ...
            with ThreadPoolExecutor(max_workers=10) as executor:
                list(
                    tqdm(
                        executor.map(download_image, missing_images, missing_file_paths),
                        total=len(missing_images),
                        desc=f"Downloading images to {manga_title} / chapter {chapter}",
                        unit="image",
                        colour="yellow",
                    )
                )
